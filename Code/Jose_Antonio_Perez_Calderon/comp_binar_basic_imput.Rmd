...
```
Obtenemos un accuracy de 0.7834197 para h1n1
...
```
Obtenemos un accuracy de 0.8022633 para seasonal
...
```
Obtenemos un accuracy de 0.8446 para h1n1
...
df <- data_frame("n"= n, "Accuracy_h1n1" = acc)
}
```
...
```{r}
df <- data_frame(df, "Accuracy_seasonal" = acc1)
plot(df$n, df$Accuracy_seasonal)
```
No observamos que tienda a estabilizarse el número de árboles, al menos con estas cantidades.
...
La puntuación obtenida en la plataforma es del 0.8254
